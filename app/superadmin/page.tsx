  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1 px-3 py-2.5 rounded-lg font-mono text-sm truncate text-right" style={{ background: "#f7f2e7", border: "1px solid #e7ddc4", color: "#16243f", direction: 'ltr' }}>
          {value}
        </div>
        <button className="icon-btn-lg" onClick={handleCopy} title="نسخ" style={{ background: 'transparent', cursor: 'pointer' }}>
...
export default function SuperAdminPanel() {
  ...
  const [resetOrg, setResetOrg] = useState<Org | null>(null);
  ...
  function toggleSuspend(org: Org) {
    setOrgs((prev) => prev.map((o) => (o.id === org.id ? { ...o, status: o.status === "suspended" ? "active" as const : "suspended" as const } : o)));
    setOpenMenu(null);
  }
  ...
  function handleResetPassword(org: Org) {
    setResetOrg(org);
  ...
  return (
    <div dir="rtl" className="superadmin-app min-h-screen flex w-full" style={{ fontFamily: "'Tajawal', sans-serif" }}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .icon-btn { width:2rem; height:2rem; display:flex; align-items:center; justify-content:center; border-radius:0.5rem; color:#6b6457; transition:all .15s ease; background:transparent; border:none; cursor:pointer; }
        .icon-btn:hover { background:#f3e6c0; color:#16243f; }
        .icon-btn-lg { width:2.4rem; height:2.4rem; flex-shrink:0; display:flex; align-items:center; justify-content:center; border-radius:0.6rem; border:1px solid #e0d6b8; color:#6b6457; background:transparent; cursor:pointer; }
        ...
        .btn-gold { background:linear-gradient(180deg,#d9b94a,#b8923a); color:#16243f; font-weight:800; border:none; cursor:pointer; box-shadow:0 6px 16px -6px rgba(184,146,58,0.6); }
        ...
        .chip { font-size:0.78rem; font-weight:600; padding:0.4rem 0.95rem; border-radius:9999px; border:1px solid #e0d6b8; color:#6b6457; cursor:pointer; transition:all .15s ease; white-space:nowrap; background:transparent; }
        .chip.active { background:#16243f; color:#e9c969; border-color:#16243f; }

        .field-label { font-size:0.75rem; font-weight:700; color:#6b6457; margin-bottom:0.4rem; display:block; text-align:right; }
        .field-input { width:100%; font-size:0.9rem; padding:0.65rem 0.8rem; border-radius:0.6rem; border:1px solid #e0d6b8; background:#fffdf8; color:#1f2733; outline:none; text-align:right; }
      ...
      <aside className="sidebar w-64 flex-shrink-0 hidden lg:flex flex-col p-4">
        <div className="flex items-center gap-3 pb-5 mb-4 border-b text-right" style={{ borderColor: "rgba(243,230,192,0.15)" }}>
        ...
        <div className="pt-4 mt-4 border-t" style={{ borderColor: "rgba(243,230,192,0.15)" }}>
          <div className="flex items-center gap-3 px-1 text-right">
            ...
            <button title="تسجيل الخروج" className="text-gray-400 hover:text-amber-300 transition-colors" style={{ background: 'transparent', border: 'none', cursor: 'pointer' }}>
              <LogOut size={16} />
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="flex items-center justify-between gap-4 px-5 lg:px-8 py-4 bg-white/70 backdrop-blur border-b text-right" style={{ borderColor: "#e7ddc4" }}>
          <div>
        ...
        <main className="content-bg flex-1 px-5 lg:px-8 py-7 overflow-y-auto text-right">
          {/* الإحصائيات */}
                        ...
                        <div className="relative flex justify-end">
                          ...
                          {openMenu === org.id && (
                            <>
                              ...
                              <div className="dropdown-menu absolute left-0 top-9 z-20 w-60 py-1.5 text-right">
          ...
          <div className="modal-card p-6" onClick={(e) => e.stopPropagation()}>
            {createStep === "form" && (
              <>
                ...
                <p className="text-xs mb-5 text-right" style={{ color: "#6b6457" }}>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input className="field-input font-mono" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} style={{ direction: 'ltr' }} />
