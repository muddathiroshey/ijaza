...
function StepCircle({ n, status }: { n: number; status: 'done' | 'active' | 'upcoming' }) {
  return (
    <div
...
function ReqItem({ ok, label }: { ok: boolean; label: string }) {
  return (
    <li className="flex items-center gap-1.5 text-xs text-right" style={{ color: ok ? "#4f7d4a" : "#a39c8c" }}>
...
export default function FirstLoginSetup() {
  ...
  const step1Valid = fullName.trim().length > 1 && email.includes("@");
  ...
  return (
    <div dir="rtl" className="onboarding-app min-h-screen flex w-full" style={{ fontFamily: "'Tajawal', sans-serif" }}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .field-label { font-size:0.78rem; font-weight:700; color:#6b6457; margin-bottom:0.4rem; display:flex; align-items:center; gap:0.35rem; text-align:right; }
        .field-input { width:100%; font-size:0.9rem; padding:0.7rem 0.85rem; border-radius:0.6rem; border:1px solid #e0d6b8; background:#fffdf8; color:#1f2733; outline:none; transition:border-color .15s ease; text-align:right; }
        .field-input:focus { border-color:#c9a227; }
        .field-with-icon { position:relative; }
        .field-with-icon .field-input { padding-right:2.5rem; }
        .field-with-icon .icon-slot { position:absolute; right:0.8rem; top:50%; transform:translateY(-50%); color:#a39c8c; background:transparent; border:none; cursor:pointer; }
      ...
      <div className="brand-panel hidden lg:flex lg:w-[42%] flex-col justify-between p-12 relative text-right">
                    ...
                    <div>
                      ...
                      {!logoName ? (
                        <label className="upload-box">
                          <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files && e.target.files[0] && setLogoName(e.target.files[0].name)} />
                        ...
                        <div className="flex items-center gap-2.5 p-3 rounded-lg" style={{ background: "#f7f2e7", border: "1px solid #e7ddc4" }}>
                          ...
                          <button className="icon-btn" style={{ width: 'auto', height: 'auto', padding: '0.2rem' }} onClick={() => setLogoName("")}>
